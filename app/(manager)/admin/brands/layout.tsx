...
const BrandsLayout = ({ children }: BrandsLayoutProps) => {
    ...
    const getBreadcrumbs = () => {
        ...
        if (segments.includes('new')) {
            breadcrumbs.push({ name: 'Thêm mới', href: '/admin/brands/new' });
        } else if (segments.length > 2) {
            const id = segments[2];
            if (segments.includes('edit')) {
                breadcrumbs.push({ name: 'Chỉnh sửa', href: `/admin/brands/${id}/edit` });
            } else {
                breadcrumbs.push({ name: 'Chi tiết', href: `/admin/brands/${id}` });
            }
        }
