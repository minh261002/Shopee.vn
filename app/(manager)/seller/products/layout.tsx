...
const ProductsLayout = ({ children }: ProductsLayoutProps) => {
    ...
    const getBreadcrumbs = () => {
        ...
        const breadcrumbs = [
            ...
            { name: 'Sản phẩm', href: '/seller/products' },
        ];

        if (segments.includes('new')) {
            breadcrumbs.push({ name: 'Thêm mới', href: '/seller/products/new' });
        } else if (segments.length > 2) {
            const id = segments[2];
            if (segments.includes('edit')) {
                breadcrumbs.push({ name: 'Chỉnh sửa', href: `/seller/products/${id}/edit` });
            } else {
                breadcrumbs.push({ name: 'Chi tiết', href: `/seller/products/${id}` });
            }
        }
